...
def write_json():
    ...
    rpid = console.input(
        "[bold cyan][INPUT][/bold cyan] [bold white]Please enter Discord Application ID[/bold white] [bold cyan](Rich "
        "Presence)[/bold cyan][bold white]: [/bold white]")
...
def status_write():
    rpcState = console.input(
        "[bold cyan][INPUT][/bold cyan] [bold white]Please enter what you would like State to become: [/bold white]")
    rpcDetails = console.input(
        "[bold cyan][INPUT][/bold cyan] [bold white]Please enter what you would like State Details to become: [/bold "
        "white]")
    rpcL_Image_Name = console.input(
        "[bold cyan][INPUT][/bold cyan] [bold white]Please enter Large Image Asset name: [/bold white]")
    rpcS_Image_Name = console.input(
        "[bold cyan][INPUT][/bold cyan] [bold white]Please enter Small Image Asset name: [/bold white]")
    status = {"rpcState": rpcState, "rpcDetails": rpcDetails, "rpcS_Image_Name": rpcS_Image_Name,
              "rpcL_Image_Name": rpcL_Image_Name, "developer": "bucky#9990"}
...
async def on_ready():
    system('cls')
    renew_check = console.input(
        "[bold yellow][IMPORTANT][/bold yellow] [bold white]Do you want to Change your Previous Status or not? (y/n) "
        "[/bold white]")
